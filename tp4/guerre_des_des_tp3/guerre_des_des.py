...
class GuerreDesDes:
    ...
    def resultat_attaque(self, attaquant, force_attaquant, defenseur, force_defenseur):
        ...
        elif force_attaquant <= force_defenseur:
    ...
    def attaque_succes(self, attaquant, defenseur):
        ...
        list_de_restant_attaquant = attaquant.des[:1]
        list_de_a_deplacer_attaquant = attaquant.des[:attaquant.nombre_de_des() - 1]

        attaquant.remplacer_des(list_de_restant_attaquant)

        defenseur.remplacer_des(list_de_a_deplacer_attaquant)
    ...
    def fin_du_tour(self, joueur):
        ...
        for _ in range(taille_territoire):
            list_de_nouveau.append(De())

        joueur.ajouter_n_des(list_de_nouveau)
    ...
    def conquete(self, joueur):
        ...
        """
        # VOTRE CODE ICI
        list_case = [case.appartenance.couleur for case in self.carte.cases.values()]

        return all(x == joueur.couleur for x in list_case)
